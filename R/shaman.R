    ...
    dist_resolution=NA, smooth=NA)
{
   ...
   shuffle_exe = sprintf("%s/%s", system.file("bin", package="shaman"), getOption("shaman.shuffle_exe"))
    ...
    commands <- paste0("{library(shaman); shuffle_hic_mat_for_track(\"", track_db, "\",\"", obs_track_nm, "\",\"", work_dir,
	...
	", dist_resolution=", dist_resolution, ", decay_smooth=",
        smooth, ", shuffle=", near_cis_shuffle, ")}")
#    res <- eval(parse(text=paste("gcluster.run(", commands, ",opt.flags=\"", sge_flags,  "\" ,max.jobs=", max_jobs, ")")))
    ...
    commands <- paste0("{library(shaman); shuffle_hic_mat_for_track(\"", track_db, "\",\"", obs_track_nm, "\",\"",
