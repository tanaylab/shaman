...
Now we will need to go to the supplied directory (`my_shaman_proj/conf` in our case) and complete the _configuration_ files.

* __shaman.conf__: Contains all settings. 

Once all configuration data has been updated, load configuration data by calling:
```{r, eval=FALSE, warning=FALSE}
...
```
![Example of sptial contact enrichment for converging CTCF pairs: log10(obs) on the left, log2(obs/exp) on the right ](/home/nettam/projects/hic/figures/paper/hg19.k562.ctcf_neg.ctcf_pos.30.1.png)
